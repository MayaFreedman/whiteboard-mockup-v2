...
>(({ tool, isActive, onClick }, ref) => (
  <SimpleTooltip content={tool.label}>
    <Button
      ...
      className={`p-2 ${
        isActive
          ? "bg-company-dark-blue text-company-dark-blue-foreground hover:bg-company-dark-blue/90"
          : ""
      }`}
    >
...
const ActionButtons: React.FC = () => {
  ...
  const handleScreenshot = async () => {
    try {
      ...
      if (
        /rgba\s*\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*0\s*\)/i.test(bg) ||
        bg === "transparent"
      ) {
        bg = "#ffffff";
      }
      ...
      const filename = `whiteboard-${d.getFullYear()}-${pad(
        d.getMonth() + 1
      )}-${pad(d.getDate())}_${pad(d.getHours())}-${pad(d.getMinutes())}-${pad(
        d.getSeconds()
      )}.png`;
...
export const Toolbar: React.FC = () => {
  ...
  const displayedShape =
    selectedShape ||
    SHAPE_TOOLS.find((shape) => shape.id === lastUsedShapeTool);
  const shouldHighlightShapeButton = isShapeSelected;

  return (
    <div
      ref={toolbarRef}
      className="bg-card border-b border-company-light-pink/20 relative"
    >
      {/* Desktop Action Buttons - Fixed to the right */}
      <div
        className={`absolute right-4 top-0 h-full items-center gap-2 z-10 ${
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="sm"
                  className={`p-2 gap-1 text-sm font-medium ${
                    isMobile ? "px-2" : ""
                  }`}
